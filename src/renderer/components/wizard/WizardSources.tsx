...
interface WizardSourcesProps {
    data: Record<string, any>
    updateData: (updates: Record<string, any>) => void
}

export function WizardSources({ data, updateData }: WizardSourcesProps) {
    ...
    const handleScanClick = async () => {
        ...
        try {
            // @ts-ignore
            await window.api.invoke('scanner:open-window')
        } catch (e: any) {
                                ...
                                <div className="flex flex-col gap-2 col-span-2 border-t border-slate-100 pt-5 mt-1">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        {source.timeRange === 'custom_range' && (
                                            <div className="flex items-center gap-3">
                                                <VintageDatePicker
                                                    selected={source.startDate ? new Date(source.startDate) : null}
                                                    onChange={(date) => updateSource(index, { startDate: date ? date.toISOString().slice(0, 10) : '' })}
                                                    placeholder="Từ ngày..."
                                                />
                                                <span className="text-slate-300 font-bold">→</span>
                                                <VintageDatePicker
                                                    selected={source.endDate ? new Date(source.endDate) : null}
                                                    onChange={(date) => updateSource(index, { endDate: date ? date.toISOString().slice(0, 10) : '' })}
                                                    placeholder="Đến ngày..."
                                                    minDate={source.startDate ? new Date(source.startDate) : undefined}
                                                />
                                            </div>
                                        )}
                                    </div>
