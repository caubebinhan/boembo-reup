...
export function TroubleshootingPanel() {
  ...
  const caseMapRef = useRef<Map<string, TroubleCase>>(new Map())
  ...
  const loadWorkflowCandidates = async (workflowId?: string) => {
    ...
    try {
      ...
      console.error('[TroubleshootingPanel] load workflow candidates failed', err)
      setVideoCandidates([])
      setSourceCandidates([])
    }
  }
  ...
  useEffect(() => {
    ...
    const offUpdate = api.on?.('troubleshooting:run-update', (payload: any) => {
      ...
      if (next.status !== 'running') {
        ...
        const caseDef = caseMapRef.current.get(next.caseId)
  ...
  const runCase = async (caseId: string): Promise<TroubleRun | null> => {
    ...
    try {
      ...
      if (run?.status && run.status !== 'running') {
        ...
        return run as TroubleRun
      }
      return (run as TroubleRun) || null
    } catch (err: any) {
      console.error('[TroubleshootingPanel] runCase failed', err)
      setBusy(prev => ({ ...prev, [caseId]: false }))
      setMessageTone('error')
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-vintage-white p-6 h-full text-vintage-charcoal">
      <div className="max-w-[1500px] mx-auto space-y-5">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg border text-sm transition ${
                  showAdvancedControls
                    ...
                    : 'border-vintage-border bg-vintage-white text-vintage-charcoal hover:border-sky-300 hover:bg-sky-50'
                }`}
              >
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg border border-sky-400 bg-sky-500 text-sm font-semibold text-white hover:bg-sky-600 transition disabled:opacity-50"
              >
                {runAllProgress.active
                  ? `Running ${Math.min(runAllProgress.done, runAllProgress.total)}/${runAllProgress.total}`
                  : 'Start Troubleshooting'}
              </button>
            </div>
          </div>
          {showAdvancedControls && (
            <div className="mt-3 rounded-xl border border-vintage-border bg-vintage-white/70 p-3 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
          ...
          {runAllProgress.active && (
            <div className="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 mt-3">
              <div className="flex items-center justify-between text-xs text-sky-700">
                <span className="font-semibold">Troubleshooting Progress</span>
        ...
        {message && (
          <div className={`rounded-lg border text-sm px-3 py-2 ${messageTone === 'info'
        ...
        {showAdvancedControls && manualTiktokRepostPickersEnabled && !activeWorkflowScope && (
        ...
        {showAdvancedControls && ((!!activeWorkflowScope && !!autoRandomSeed.trim()) || (manualPickersEnabled && (selectedAccountId !== 'auto' || selectedVideoId !== 'auto' || selectedSourceId !== 'auto'))) && (
        ...
        <div className="rounded-xl border border-vintage-border bg-vintage-cream/30 p-4">
          ...
          {latestOutcome ? (
            ...
            <p className="text-sm text-vintage-gray">No finished run yet. Start troubleshooting or run an individual case to see insight.</p>
          )}
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-4">
            <div className="rounded-xl border border-vintage-border bg-vintage-cream/35 p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg border border-vintage-border bg-vintage-white text-xs text-vintage-charcoal hover:border-sky-300 hover:bg-sky-50 transition"
                  >
                    {showAllCases ? 'Hide Cases' : 'View All Cases'}
                  </button>
                </div>
              </div>
              ...
              {showAllCases ? (
                                ...
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <div className="flex items-center gap-2 flex-wrap">
                                      ...
                                      <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded border ${c.risk === 'real_publish'
                                          ? 'text-rose-700 border-rose-200 bg-rose-50'
                                          : 'text-emerald-700 border-emerald-200 bg-emerald-50'
                                        }`}>
                                        {c.risk === 'real_publish' ? 'Real Publish' : 'Safe'}
                                      </span>
                                      <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded border ${c.implemented === false
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 mt-2">
                                      ...
                                      {c.level && (
                                        <span className={`text-[10px] px-2 py-0.5 rounded border ${c.level === 'basic'
                ...
                <div className="rounded-lg border border-vintage-border bg-vintage-white/70 px-3 py-2 text-sm text-vintage-gray">
                  Case list is hidden. Click <span className="font-semibold text-vintage-charcoal">View All Cases</span> when you need to run an individual case.
                </div>
              )}
            </div>
                ...
                {filteredRuns.map(run => {
                  ...
                  return (
                    <button
                      ...
                      data-run-id={run.id}
                      className={`w-full text-left rounded-lg border px-3 py-2 transition ${selectedRunId === run.id
          ...
          <div className="rounded-xl border border-vintage-border bg-vintage-cream/35 p-4 min-h-[600px] flex flex-col lg:sticky lg:top-4 lg:max-h-[calc(100vh-3rem)] lg:overflow-y-auto">
