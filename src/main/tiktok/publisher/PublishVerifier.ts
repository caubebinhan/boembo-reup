        ...
        try {
            ...
            for (let d = 0; d < count; d++) {
                ...
                const nonViolationConfirmKeywords = [
                    ...
                    'post now',
                    '謚慕ｨｿ縺ｫ騾ｲ縺ｿ縺ｾ縺吶°',
                    '闡嶺ｽ懈ｨｩ萓ｵ螳ｳ縺ｮ繝√ぉ繝・け縺悟ｮ御ｺ・＠縺ｦ縺・∪縺帙ｓ',
                    '莉翫☆縺先兜遞ｿ',
                ]
        ...
        try {
            ...
            for (let check = 1; check <= 5; check++) {
                ...
                if (uiStatus?.id) {
                    ...
                    // Build proper public URL from username+videoId 窶・do NOT use raw href from Studio DOM
    ...
    private deriveDashboardReviewState(item: any): {
        ...
        for (const [key, raw] of candidates) {
            ...
            if (
                ...
                normalized.includes('螳｡譬ｸ') ||
                normalized.includes('蟇ｩ譬ｸ')
            ) {
                return {
                    isReviewing: true,
                    source: `api:${key}`,
