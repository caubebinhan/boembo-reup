...
export const publishCases: TroubleshootingCaseDefinition[] = [
  ...
  ttCase({
    ...
    description: 'Network drops mid-upload; engine retries or marks failed cleanly without partial corrupt publish_history row.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['No partial/orphaned publish_history row left in failed state', 'campaign video status reflects failed or retried correctly'],
        logs: ['Timeout stage + bytes-sent logged', 'FlowEngine network error handler triggered (handleNetworkError)'],
  ...
  ttCase({
    ...
    description: 'Download phase detects low disk space before writing video file; campaign fails gracefully with user-facing alert.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['campaign status=failed with disk error reason', 'No corrupt video file written to disk'],
        logs: ['Pre-run health check: disk space check triggered (ServiceHealthMonitor)', 'handleDiskError called with campaign context'],
        events: ['campaign:failed with disk-space error class emitted'],
      },
      passMessages: ['Disk space guard prevents partial video write and campaign is fail-safely terminated'],
    }),
  }),
  ttCase({
    id: 'tiktok-repost-v1.publish.caption-too-long-truncate',
    title: 'Publish Caption Exceeds TikTok 2200-Char Limit',
    description: 'Caption derived from template or source description exceeds TikTok character limit; verify truncation and publish still succeeds.',
    risk: 'real_publish',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['Stored generated_caption is truncated to limit', 'No duplicate-caption row from retry'],
  ...
  ttCase({
    ...
    description: 'Local video file was deleted between download and publish step; publisher detects missing file and marks job failed.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
  ...
  ttCase({
    ...
    description: 'Downloaded video file is < 50KB; publisher rejects upload attempt and marks video for re-download or failed.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['campaign video status=failed with corrupted_file reason', 'Corrupt cache file is deleted before retry'],
  ...
  ttCase({
    ...
    description: 'Campaign has 3 accounts; same video publishes to all 3 sequentially respecting per-account concurrency keys.',
    risk: 'real_publish',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['3 publish_history rows created with distinct account_id', 'No concurrent publish for same account'],
        logs: ['Per-account publish sequence logged with account handle + order'],
        events: ['video:published emitted 3× with distinct accountId fields'],
      },
  ...
  ttCase({
    ...
    description: 'Dedup check is per-account; same source video can be published to account A but skip if already published to account B same day.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['publish_history dedup lookup is scoped to (source_platform_id, account_id)', 'Second account proceeds independently'],
        logs: ['Per-account dedup hit/miss logged separately'],
      },
      passMessages: ['Dedup is scoped correctly: cross-account does not block independent publishes'],
    }),
  }),
  ttCase({
    id: 'tiktok-repost-v1.publish.cookie-refresh-mid-publish',
    title: 'Publish: Cookie Refresh Triggers Mid-Upload',
    description: 'TikTok redirects to re-auth partway through upload session; publisher detects redirect and surfaces session-expired instead of upload error.',
    risk: 'real_publish',
    category: 'publish',
    group: 'publish',
  ...
  ttCase({
    ...
    description: 'Upload progress indicator freezes at <100% for >60s; publisher times out and surfaces recoverable error.',
    risk: 'real_publish',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        ...
        logs: ['Progress stall time and last % logged', 'Timeout stage clearly identified in publisher logs'],
  ...
  ttCase({
    ...
    description: 'Campaign privacy param propagates correctly to TikTok Studio upload form; verify all three modes are selectable.',
    risk: 'real_publish',
    category: 'publish',
    group: 'publish',
  ...
  ttCase({
    ...
    description: 'Immediately after publish, view/like/comment counts start at 0; verify rescan later updates counters without duplicating video row.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['Initial publish_history views=0 or null', 'Rescan updates counters without creating duplicate row by platform_id'],
  ...
  ttCase({
    ...
    description: 'Two schedule slots compute the same publish time concurrently; engine deduplicates and executes only once.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['Only one publish_history row created per slot overlap', 'No duplicate video status updates'],
  ...
  ttCase({
    ...
    description: 'TikTok Studio changes "Post" button selector; publisher fails with clear selector-drift error and dumps diagnostic HTML + screenshot.',
    risk: 'safe',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        ...
        logs: ['Selector name that drifted is in log', 'Fallback selectors tried and failed are listed'],
        files: ['Debug HTML + screenshot dumped via DebugHelper.dumpPageState'],
      },
      errorMessages: ['Selector drift outputs: tried selectors list + URL at failure + artifact paths'],
    }),
  }),
  ttCase({
    id: 'tiktok-repost-v1.publish.caption-hashtag-injection',
    title: 'Publish Caption Hashtag Injection from Template',
    description: 'Caption template includes [Tags] placeholder; verify hashtag list is injected and appears in final submitted caption.',
    risk: 'real_publish',
    category: 'publish',
    group: 'publish',
    ...
    meta: meta({
      ...
      checks: {
        db: ['generated_caption contains expanded hashtag list', 'Template variables all substituted (no raw [Tags] remaining)'],
