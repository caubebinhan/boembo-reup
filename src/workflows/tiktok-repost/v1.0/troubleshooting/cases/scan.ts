...
export const scanCases: TroubleshootingCaseDefinition[] = [
  ...
  ttCase({
    ...
    description: 'TikTok API returns 429 (rate limit) during scan; verify scanner backs off, retries, and surfaces rate-limit alert.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
  ...
  ttCase({
    ...
    description: 'Private account or typo in channel URL produces zero results; scanner exits cleanly without error crash.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
  ...
  ttCase({
    ...
    description: 'Keyword scan produces videos from multiple creators; author filter configured to exact @handle reduces to expected subset.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
    ...
    meta: meta({
      ...
      checks: {
        db: ['All persisted videos have author === @specific_creator', 'Videos from other authors are excluded from campaign'],
  ...
  ttCase({
    ...
    description: 'withinDays filter boundary: videos posted at 23:59 yesterday vs 00:01 today treated correctly by date comparison.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
  ...
  ttCase({
    ...
    description: 'historyLimit set to 500; verify pagination continues to final page without infinite loop or memory spike.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
  ...
  ttCase({
    ...
    description: 'Campaign has two identical source channels; verify scanner deduplicates scanned videos at ingestion, not just display.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
    ...
    implemented: false,
    meta: meta({
      parameters: [{ key: 'fixtureDuplicateSource', value: 'same channel URL ×2 in sources[]' }],
      checks: {
        db: ['campaign.videos has no duplicate platform_id after scanning both sources', 'Dedup is applied at store level'],
  ...
  ttCase({
    ...
    description: 'Campaign has both a keyword source and a channel source; results merge correctly with dedup by platform_id.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
    ...
    meta: meta({
      ...
      checks: {
        db: ['Merged video list has no duplicate platform_id from different sources', 'Each video retains source attribution metadata'],
  ...
  ttCase({
    ...
    description: 'End-to-end: scan source, apply filters, download thumbnail, queue video, publish. Verify each handoff preserves required fields.',
    risk: 'real_publish',
    category: 'scan',
    group: 'scan',
    ...
    meta: meta({
      ...
      checks: {
        db: ['Scanned video platform_id exists in publish_history after pipeline completes', 'local_path, thumbnail, description all persisted at each node handoff'],
  ...
  ttCase({
    ...
    description: 'TikTok page loads partially (JS timeout); scanner detects incomplete page and retries or exits cleanly.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
  ...
  ttCase({
    ...
    description: 'minViews=9000 and maxViews=1000 (impossible range); scanner returns nothing and logs the contradiction without crashing.',
    risk: 'safe',
    category: 'scan',
    group: 'scan',
    ...
    meta: meta({
      ...
      checks: {
        db: ['Zero rows inserted (no video passes impossible range)', 'Campaign remains in valid state'],
      ...
      passMessages: ['Contradictory filter range handled gracefully: zero results, clear log message'],
    }),
  }),
]
